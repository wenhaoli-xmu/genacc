import torch
import types
from .modify_llama import do_sdpa_attn, do_draft_attn_via_down_proj, generate_mask, do_causal_flash_attn
...
def self_attn_forward(
    ...
    if cond1 and cond2:
        ...
        draft_score, true_score = do_draft_attn_via_down_proj(
            query=ques, key=keys,
            q_down=self.query_down_proj,
            k_down=self.key_down_proj,
            cos=cos, sin=sin,
            return_true_score=self.draft_kwargs['bench_mark'] or attn_supervise,
