...
def self_attn_forward(
    ...
    if cond1 and cond2:
        ...
        def get_attn_score_using_isolated_rope(hidden_states, cos, sin):
            q_proj = self.q_proj.weight.data @ self.q_down @ self.q_up
            k_proj = self.k_proj.weight.data @ self.k_down @ self.k_up

            query = (hidden_states @ q_proj.T).unflatten(-1, (num_heads, head_dim)).transpose(-2,-3)
            key = (hidden_states @ k_proj.T).unflatten(-1, (num_heads, head_dim)).transpose(-2,-3)

            info = {"device": query.device, "dtype": query.dtype}            
            ...
            iso_attn = iso_que @ iso_key.transpose(-1,-2)

            return query @ key.transpose(-1,-2) + iso_attn * self.gamma

        true_score = get_attn_score(query=ques, key=keys, cos=cos, sin=sin)
        draft_score = get_attn_score_using_isolated_rope(hidden_states, cos=cos, sin=sin)
...
class Decoder(torch.nn.Module):
    ...
    def __init__(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            layer.self_attn.gamma = torch.full((1, 32, 1, 1), 0.4, **info)

            reduce_mat = get_reduce_mat(layer.self_attn.q_proj.weight.data, rank)
            layer.self_attn.q_down = reduce_mat
            layer.self_attn.q_up = reduce_mat.T

            reduce_mat = get_reduce_mat(layer.self_attn.k_proj.weight.data, rank)
            layer.self_attn.k_down = reduce_mat
            layer.self_attn.k_up = reduce_mat.T
        ...
        for layer in self.layers:
            if not layer.self_attn.is_fix_layer:
                params += [
                    ...
                    layer.self_attn.q_up,
                    layer.self_attn.k_down,
                    layer.self_attn.k_up,
                ]
