...
def self_attn_forward(
    ...
    if cond1 and cond2:
        ...
        def get_attn_score_using_lsh(query, key, cos, sin):
            query, key = check_and_apply_qk_rope(query, key, cos, sin)

            query_hash = (query @ self.rand_mat) > 0
            key_hash = (key @ self.rand_mat) > 0

            """
            query_hash: (1, 32, n, 16)
            key_hash: (1, 32, n, 16)
            ------------------------
            """

            accum = torch.zeros((1,32,query_hash.shape[-2],key_hash.shape[-2]), dtype=query.dtype, device=query.device)
            for x, y in zip(torch.chunk(query_hash, query_hash.shape[-1], dim=-1), torch.chunk(key_hash, key_hash.shape[-1], dim=-1)):
                accum += torch.logical_not(torch.logical_xor(x, y.transpose(-1,-2))).type(accum.dtype)

            return accum


        true_score = get_attn_score(query=ques, key=keys, cos=cos, sin=sin)
        draft_score = get_attn_score_using_lsh(query=ques, key=keys, cos=cos, sin=sin)
