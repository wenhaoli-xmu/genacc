...
def self_attn_forward(
    ...
    if cond1 and cond2:
        ...
        def get_attn_score_using_down_proj(query, key, q_down, k_down, cos, sin, gamma=1024):
        ...
        draft_score = get_attn_score_using_down_proj(ques, keys, self.q_down, self.k_down, cos, sin)
...
class Decoder(torch.nn.Module):
    ...
    def __init__(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if not layer.self_attn.is_fix_layer:
                layer.self_attn.q_down = torch.nn.Parameter(torch.empty((1,32,128,8), **info))
                layer.self_attn.k_down = torch.nn.Parameter(torch.empty((1,32,128,8), **info))
                torch.nn.init.xavier_uniform_(layer.self_attn.q_down.data)
                torch.nn.init.xavier_uniform_(layer.self_attn.k_down.data)
    ...
    def ft_params(self):
        ...
        for layer in self.layers:
            if not layer.self_attn.is_fix_layer:
                params += [layer.self_attn.q_down, layer.self_attn.k_down]
