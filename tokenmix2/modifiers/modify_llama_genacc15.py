...
def self_attn_forward(
    ...
    if cond1 and cond2:
        ...
        def get_attn_score_using_angle_lsh_train_mode(query, key, rot_mat, cos, sin, gamma=64):
        ...
        def log_diffs(true_attn, draft_attn):
            ...
            oth_rnd_indices = torch.randperm(indices.shape[-1] - top_cnt, dtype=torch.int64, device=indices.device)[:1024]
        ...
        def get_attn_score_using_angle_lsh(query, key, rot_mat, cos, sin):
            query, key = check_and_apply_qk_rope(query, key, cos, sin)
            q_hash = torch.sign(query @ rot_mat)
            k_hash = torch.sign(key @ rot_mat)
            sim = q_hash @ k_hash.transpose(-1,-2)
            return sim

        draft_score = get_attn_score_using_angle_lsh(query=ques, key=keys, rot_mat=self.rot_mat, cos=cos, sin=sin)
